<template>
<div>
  <gmap-map v-show="showMap" ref="map" :center="coords" :zoom="zoom" style="width: 70%; height: 500px">
    <gmap-marker :position="this.coords"></gmap-marker>
    <gmap-marker :position="this.destination"></gmap-marker>
    <gmap-marker v-for="(camp, index) in campCoordinates" :key="index" :position="camp.location" :title="camp.name" :clickable="true" :draggable="false" v-on:click="openInfoWindow(camp)" />
  </gmap-map>
  <div class="find-button">
    <button v-if="this.$store.state.tripCalculated" @click="splitUpTrip" type="button" class="btn btn-success">
      Find Campsites
    </button>
  </div>
</div>
...
<script>
export default {
  props: ["dailyDriveTime", "allCampsites", "tripInfo", "savedView"],
  data() {
    return {
      showMap: true,
      zoom: 5,
  ...
  methods: {
    ...
    splitUpTrip: function () {
      ...
      this.daysOfDriving = Math.ceil(
        this.tripLength.tripDuration / dailyDriveInSeconds
      );
      let timeTracker = 0;
      let dayNumber = 1;
